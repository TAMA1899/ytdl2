...
from config import youtube_next_fetch, UPDATES_CHANNEL
...
async def ytdl(c, message):
    if UPDATES_CHANNEL:
      fsub = await handle_force_subscribe(c, message)
      if fsub == 400:
        return 
    ...
    try:
        ...
        await message.reply_text("`Proses Gagal... 😔 \nPossible Youtube Blocked server ip \n#error`")
        return
    buttons = InlineKeyboardMarkup(list(create_buttons(formats)))
    sentm = await message.reply_text("🔎 Sedang Memproses")
    try:
